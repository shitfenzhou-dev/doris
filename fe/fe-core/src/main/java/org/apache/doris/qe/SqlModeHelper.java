...
public class SqlModeHelper {
    ...
    public static Long encode(String sqlMode) throws DdlException {
        List<String> names = Splitter.on(',').trimResults().omitEmptyStrings().splitToList(sqlMode);

        long resultCode = 0L;
        for (String key : names) {
            long code = 0L;
            if (VariableVarConverters.hasLongFormat(key)) {
                code |= expand(VariableVarConverters.parseNonNegativeLongForVar(key, SessionVariable.SQL_MODE));
            } else {
